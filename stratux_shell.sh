...
if [ "$MACHINE" == "$ARM6L" ] || [ "$MACHINE" == "$ARM7L" ]; then
    ...
    wget https://storage.googleapis.com/golang/go1.10.3.linux-armv6l.tar.gz --no-check-certificate
    tar -zxvf go1.10.3.linux-armv6l.tar.gz
...
##############################################################
##  WiringPi tools build
##############################################################

echo
echo "${YELLOW}**** WiringPi Install ****${WHITE}"
cd /tmp
